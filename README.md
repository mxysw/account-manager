...
6. 成功后，账号行会在邮箱上方显示“两步验证手机号”：页面只显示脱敏号码，点击脱敏号码才会复制完整号码。标签会区分“本次添加”“原本已有”“免短信验证”和“待生效”；历史记录无法确认来源时只显示“已绑定”，不会猜测。
7. 共享模式下，无论本账号成功还是失败，任务结束都会解除本次占用；号码立即重新显示为可用，不会因为一个失败账号卡住整批任务。

如果 Google 另行要求短信验证码，程序不会把账号的身份验证器动态码误填进去；当前账号会结束并记录原因，共享号码随即交给下一个账号继续尝试。

两种模式的区别：

- **共享号码**：同一个号码可依次尝试任意多个账号。成功会累计“已绑 N”；失败、超时、短信验证或人机验证也会立刻结束本账号并把号码交给下一个账号。本地手机号池不限制共享复用次数。Google 自身对同一号码的绑定次数、频率和账号风险有独立限制，仍可能要求验证或拒绝，这不属于本地次数限制。
- **一号一绑**：一个号码只分配给一个账号；成功使用后显示“一号一绑·已用”，不再进入可领取集合。

已绑定的号码仍可在手机号池中勾选和管理。共享号码可以在状态下拉框中选择“停用复用”，需要继续分配时再选择“恢复共享可用”。任务正在运行时会短暂显示“已占用”；任务结束即自动释放。若旧任务异常中断，服务重启会自动恢复，也可在号码行点击“释放共享占用”。删除已有绑定记录时页面会再次确认，并向后端明确请求强制删除绑定记录；该操作**只删除本地手机号池记录，不会从 Google 账号中解绑手机号**。
...
- 勾选“跑完不关闭窗口”时，普通任务会保留浏览器供人工检查；“添加两步验证手机号”批次会忽略该全局选项，以免首个账号占住唯一并发位。共享号码失败后会关闭本账号窗口并继续下一账号。
